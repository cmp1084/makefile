...
void _init_startup(void)
{
	...
	#if configDBG
	{
		...
		{
		    /* The baud rate, maximum possible at 115 kHz is 9600 */
		...
		usart_init_rs232( configDBG_USART, &opt, FOSC0 );
	}
	#endif
}
/*-----------------------------------------------------------*/
...
static void prvSetupTimerInterrupt(void)
{
	...
	#if( configTICK_USE_TC==1 )
	{
		INTC_register_interrupt(&vTick, configTICK_TC_IRQ, AVR32_INTC_INTLEVEL_INT0);
	...
	#else
	{
		INTC_register_interrupt(&vTick, AVR32_CORE_COMPARE_IRQ, AVR32_INTC_INTLEVEL_INT0);
